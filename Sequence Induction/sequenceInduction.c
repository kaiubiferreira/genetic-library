...
void printGene(struct tNode * node)
{
    switch(node->value)
    {
        ...
        case X: printf("X ");
            break;
        case CONST: printf("%d ", node->constant);
            break;
    }
}

float calcSample(struct tNode * node, int sample)
{
    float value1 = 0;
    float value2 = 0;
    ...
    switch (node->value)
    {
        ...
        case X: return sample;
            break;
        case CONST: return node->constant;
            break;
    }
...
float rawFitness(struct tNode * node)
{
    ...
    for(i = 0; i < 20; i++)
    {
        ...
        sum += fabs(result - (5*sample*sample*sample*sample+4*sample*sample*sample+3*sample*sample+2*sample+1)); // Compares the function created with the root sidentity
        sample += 1; // Sets the interval between the samples, in this case 20 samples from [-1 to 1]
    }
