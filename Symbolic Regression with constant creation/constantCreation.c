...
struct tNode * createOrganism(int maxHeight, int currentHeight, int blnFull, struct tNode * parent)
{
    ...
    if(node->value == CONST)
    {
        node->constant = rand()%5 + rand()/(double)RAND_MAX;
    }
...
void printGene(struct tNode * node)
{
    switch(node->value)
    {
        ...
        case X: printf("X ");
            break;
        case CONST: printf("%f ", node->constant);
            break;
    }
}
...
float rawFitness(struct tNode * node)
{
    ...
    for(i = 0; i < 20; i++)
    {
        ...
        sum += fabs(result - (M_E*sample*sample + M_PI*sample)); // Compares the function created with the root sidentity
        sample += 0.10526315; // Sets the interval between the samples, in this case 20 samples from [-1 to 1]
    }
